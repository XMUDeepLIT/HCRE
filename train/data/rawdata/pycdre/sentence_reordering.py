import random
from pycdre.topological_sort import Graph


class SentReOrdering():
    
    def __init__(self, doc1_sentences, doc2_sentences, tokenizer, h, t, sbert_wk):
    ...
    def half_sbert_encoding(self):
        ...
        for i, sent_prior in enumerate(self.doc1_sentences):
            for j, sent_later in enumerate(self.doc2_sentences):
                similarity = self.sbert.pair_sims(
                    " ".join(sent_prior), " ".join(sent_later))
    ...
    def generate_edges(self):
        ...
        for s_2_idx in range(doc2_num):
            s_2_map = list(
                (filter(lambda pair: pair[1] == s_2_idx, pairs_1_with_2)))
            head_idx = sorted(
                s_2_map, key=lambda sims: sims[2], reverse=True)[0][0]
            Edge_2_to_1.append((s_2_idx, '->', head_idx))
        for s_1_idx in range(doc1_num):
            s_1_map = list(
                (filter(lambda pair: pair[1] == s_1_idx, pairs_2_with_1)))
            head_idx = sorted(
                s_1_map, key=lambda sims: sims[2], reverse=True)[0][0]
    ...
    def all_sort(self, starts, ends):
        ...
        for s_e_pair in s_e_pairs:
            ...
            chain.append(start)
            peers = list(
                (filter(lambda pair: pair[0] == start and pair[1] not in chain, pairs)))
            next_blk = sorted(
                peers, key=lambda sims: sims[2], reverse=True)[0][1]
            while next_blk != end:
                chain.append(next_blk)
                peers = list(
                    (filter(lambda pair: pair[0] == next_blk and pair[1] not in chain, pairs)))
                next_blk = sorted(
                    peers, key=lambda sims: sims[2], reverse=True)[0][1]
            chain.append(next_blk)
            chains.append(chain)
    ...
    def semantic_based_sort(self, starts, ends):
        num_total_sentences = len(self.sentences)
        ...
        peers = list(
            filter(
                lambda pair: pair[0] == start and pair[1] not in chain, 
                pairs
            )
        )
        ...
        while next_blk not in ends:
            ...
            peers = list(
                filter(
                    lambda pair: pair[0] == next_blk and pair[1] not in chain, 
                    pairs
                )    
            )
            next_blk = sorted(peers, key=lambda sims: sims[2], reverse=True)[0][1]
        chain.append(next_blk)
        rest = list(set(range(num_total_sentences)) - set(chain))
        random.shuffle(rest)
        if len(chain) < 20:
            chain = chain[:-1] + rest[: min(20, num_total_sentences) - len(chain)] + chain[-1:]
        return [ chain ]
    ...
    def unsemantic_based_sort(self, starts, ends):
        ...
        sentences = self.doc1_sentences + self.doc2_sentences
        s_ids = list(set([idx for idx, _ in enumerate(
            sentences)]).difference(set([start, end])))
    ...
    def bidirection_sort(self, starts, ends):
        ...
        for s_e_pair in s_e_pairs:
            ...
            peers_head = list(
                (filter(lambda pair: pair[0] == start and pair[1] not in chain_head, pairs_head)))
            next_blk_head = sorted(
                peers_head, key=lambda sims: sims[2], reverse=True)[0][1]
            peers_tail = list(
                (filter(lambda pair: pair[0] == end and pair[1] not in chain_tail, pairs_tail)))
            next_blk_tail = sorted(
                peers_tail, key=lambda sims: sims[2], reverse=True)[0][1]
            while next_blk_head != end and len(chain_head) < 4:
                chain_head.append(next_blk_head)
                pairs_head = self.peer_encoding(next_blk_head)
                peers_head = list((filter(
                    lambda pair: pair[0] == next_blk_head and pair[1] not in chain_head, pairs_head)))
                next_blk_head = sorted(
                    peers_head, key=lambda sims: sims[2], reverse=True)[0][1]
            while next_blk_tail != start and len(chain_tail) < 4:
                chain_tail.append(next_blk_tail)
                pairs_tail = self.peer_encoding(next_blk_tail)
                peers_tail = list((filter(
                    lambda pair: pair[0] == next_blk_tail and pair[1] not in chain_tail, pairs_tail)))
                next_blk_tail = sorted(
                    peers_tail, key=lambda sims: sims[2], reverse=True)[0][1]
            if next_blk_head == end or next_blk_tail == start:
                ...
                chain_tail.reverse()
                chain = merge_chain(chain_head=chain_head,
    ...
    def threeSent(self, starts, ends, co_occur):
        ...
        def multihop_path(starts, ends, co_occur):
            ...
            end_pos = {}
            start_edges = list(
                (filter(lambda co: co[0] == 1 and co[2] in starts, co_occur)))
            end_edges = list(
                (filter(lambda co: co[0] == 2 and co[2] in ends, co_occur)))
            ...
            co_occur = list(set(co_occur).difference(
                set(start_edges)).difference(set(end_edges)))
            ...
            while (len(next_set.intersection(pre_set)) == 0 and len(co_occur) > 0):
                start_edges = list(
                    (filter(lambda co: co[0] in list(next_set), co_occur)))
                end_edges = list(
                    (filter(lambda co: co[0] in list(pre_set), co_occur)))
                co_occur = list(set(co_occur).difference(
                    set(start_edges)).difference(set(end_edges)))
            ...
            path_edges = list(
                (filter(lambda co: co[0] in entity_chain and co[1] in entity_chain, edges_tuple)))
            path_triplet = list(
                (filter(lambda e: (e[0], e[1]) in path_edges, ori_co_occur)))
        ...
        def default_path(starts, ends, co_occur, max_pos):
            chain = starts + ends
            if len(chain) >= 8:
                chain = [starts[0]] + [ends[0]] + random.choices(
                    list(set(starts+ends).difference(set([starts[0]] + [ends[0]]))), k=6)
            else:
                while (len(chain) <= 7):
                    offset = [-3, -2, -1, 1, 2, 3]
                    ex_blk = random.choices(chain, k=1)[
                        0] + random.choices(offset, k=1)[0]
