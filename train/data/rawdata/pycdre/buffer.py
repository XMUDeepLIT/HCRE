...
class Block:
    ...
    def __init__(self, ids, pos, tokenizer, blk_type=1, **kwargs):
        self.ids = ids
        self.pos = pos
    ...
    def __lt__(self, rhs):  # rhs, right-hand-side
...
class Buffer:
    ...
    @staticmethod
    def split_document_into_blocks(d, tokenizer, capacity, cnt=0, hard=True, properties=None, docid=0):
        ret = Buffer(capacity)
        ...
        if hard:
            for sid, tsen in enumerate(d):
                ...
                for i in range(num):
                    ...
                    ret.insert(
                        Block(tokenizer.convert_tokens_to_ids(tmp), cnt, tokenizer, **tmp_kwargs)
                    )
        else:
            ...
            poses = [(i, end_tokens[tok])
                     for i, tok in enumerate(d) if tok in end_tokens]
            poses.insert(0, (-1, 0))
    ...
    def __add__(self, buf):
        assert self.capacity == buf.capacity
        ret = Buffer(self.capacity)
    ...
    def filtered(self, fltr: Callable[[Block, int], bool], need_residue=False):
        ret, ret2 = Buffer(self.capacity), Buffer(self.capacity)
    ...
    def fill(self, buf):
        ...
        for blk in buf:
            if tmp_size + len(blk) > self.capacity:
                ret.append(tmp_buf)
    ...
    def export_01_turn(self, device=None, length=None, out=None):
        ...
        for b in self.blocks:
            try:
                ids[t: t + len(b)] = torch.tensor(b.ids, dtype=torch.long, device=device)  # token id
            except:
                # pdb.set_trace()
        ...
        for i in range(len(ids_list)):
            if ids_list[i] == 102:  # 102: <sep>
                sentences_with_sep.append(ids_list[s_ptr: i + 1])
                s_ptr = i + 1

        s_ptr = 0
        for s_idx in range(len(sentences_with_sep)):
            len_cur_sent = len(sentences_with_sep[s_idx])
            type_ids[s_ptr: s_ptr + len_cur_sent] = torch.tensor([s_idx % 2] * len_cur_sent, dtype=torch.long, device=device)
            s_ptr += len_cur_sent
    ...
    def export_01_doc(self, device=None, length=None, out=None):
        if out is None:
            if length is None:
                ...
                total_length = length * len(self.blocks)
            ids, att_masks, type_ids = torch.zeros(
                3, total_length, dtype=torch.long, device=device)
        else:  # must be zeros and big enough
            ids, att_masks, type_ids = out
            att_masks.zero_()
        t = 0
        ...
        # pdb.set_trace()
        ids[:len(doc0_ids)] = torch.tensor(
            doc0_ids, dtype=torch.long, device=device)
        try:
            ids[len(doc0_ids):len(doc0_ids)+len(doc1_ids)
                ] = torch.tensor(doc1_ids, dtype=torch.long, device=device)
        except:
            pdb.set_trace()
            print(doc1_ids)
            doc1_ids.reverse()
        ...
        type_ids[:len(doc0_ids)] = torch.tensor(
            [0]*len(doc0_ids), dtype=torch.long, device=device)
        type_ids[len(doc0_ids):len(doc0_ids)+len(doc1_ids)] = torch.tensor([1]
                                                                           * len(doc1_ids), dtype=torch.long, device=device)
        return ids, att_masks, type_ids
